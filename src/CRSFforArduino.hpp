...
namespace crsf_for_arduino
{
    ...
    private:
        ...
        // CRC8 table for CRSF based on polynomial 0xD5.
        ...
        // CRC8 calculation variables.
        unsigned char computed_crc = 0;
        unsigned char received_crc = 0;

        // CRC8 function.
        auto crc8(const unsigned char offset, const unsigned char *data, unsigned char length) -> unsigned char
        {
            unsigned char crc = CRC8_TABLE[0 ^ data[offset]];
            for (unsigned char i = offset + 1; i < length; ++i)
            {
